...
class ClientService
{
    ...
    {
        $data['type'] = \App\Enums\Partner::Client;
    ...
    {
        return Datatables::of($this->partnerRepository->getDatatableRecordsByType(\App\Enums\Partner::Client))
            ->addColumn('actions', function(Partner $partner) {
                $actions = [
                    'edit' => route('clients.edit', ['client' => $partner->id]),
                    'delete' => route('clients.destroy', ['client' => $partner->id])
                ];
